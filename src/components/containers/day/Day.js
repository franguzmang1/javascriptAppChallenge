...
class Day extends Component {
      ...
      this.setState(
        {
          ...
          endOfLunch: snap.val().endOfLunch,
          leavingTime: snap.val().leavingTime,
        ...
        () => {
          ...
          date2.setHours(
            this.state.leavingTime.substring(0, 2),
            this.state.leavingTime.substring(3, 5),
            0
          );

          date1.setHours(
            this.state.arrivalTime.substring(0, 2),
            this.state.arrivalTime.substring(3, 5),
            0
          );
          ...
          date22.setHours(
            this.state.endOfLunch.substring(0, 2),
            this.state.endOfLunch.substring(3, 5),
            0
          );

          date11.setHours(
            this.state.startOfLunch.substring(0, 2),
            this.state.startOfLunch.substring(3, 5),
            0
          );
        ...
        */
          ...
          this.setState({ timeWorking: f, timeInLunch: f2 });
  ...
  calculateHours(
    ...
    date2.setHours(
      leavingTime1.substring(0, 2),
      leavingTime1.substring(3, 5),
      0
    );

    date1.setHours(
      arrivalTime1.substring(0, 2),
      arrivalTime1.substring(3, 5),
      0
    );
    ...
    if (!workingHoursIsValid) {
      //if there's nothing in validation text 1, set the value in the state normally
      this.setState({ timeWorking: f });
    } else {
      //if there's something in validation text set the variables to null and set 'timeWorking' to '00:00'
      f = null;
      diff = null;
    ...
    let date2Lunch = new Date();
    let date1Lunch = new Date();

    date2Lunch.setHours(
      endLunch1.substring(0, 2),
      endLunch1.substring(3, 5),
      0
    );

    date1Lunch.setHours(
      startLunch1.substring(0, 2),
      startLunch1.substring(3, 5),
      0
    );
    ...
    if (!lunchHoursIsValid) {
      //if there's nothing in validation text 1, set the value in the state normally
      this.setState({ timeInLunch: f2 });
    } else {
      //if there's something in validation text set the variables to unll and set 'timeInLunch' to '00:00'
      f2 = null;
      diff2 = null;
  ...
  arrivalTimeChanged(event) {
    ...
    if (event.target.value <= this.state.leavingTime) {
      ...
      //start time is greater than leaving time
    ...
    this.calculateHours(
      ...
      textInValidation1,
      this.state.startOfLunch,
      this.state.endOfLunch,
      this.state.validationText2
    );
  }
  ...
  leavingTimeChanged(event) {
    ...
    if (event.target.value >= this.state.arrivalTime) {
      ...
      this.setState({
        leavingTime: event.target.value,
        validationText1: textInValidation1,
      });
    } else {
      //leaving time is less than arrival time
      textInValidation1 = "Start time is greater than leaving time";
  ...
  startLunchChanged(event) {
    ...
    if (event.target.value <= this.state.endOfLunch) {
      textInValidation2 = "";
      this.setState({
        startOfLunch: event.target.value,
  ...
  render() {
    ...
    let validationP1 = this.state.validationText1 ? (
      ...
      <p className={classes.informationText}>
        Time is inputted in 24-hour format{" "}
      </p>
    );
    ...
    if (this.state.totalWorkedHours === this.state.expectedHours) {
      ...
      date2.setHours(
        this.state.totalWorkedHours.substring(0, 2),
        this.state.totalWorkedHours.substring(3, 5),
        0
      );

      date1.setHours(
        this.state.expectedHours.substring(0, 2),
        this.state.expectedHours.substring(3, 5),
        0
      );
      ...
      textToDisplay = (
        <p>You surpassed today's hours requirement by {f} hours!! </p>
      );
    } else {
      //the hours requirement was not met
      ...
      textToDisplay = (
        <p>
          You did not meet today's hours requirement, there are {f} hours left
          to complete on this day
        </p>
      );
    }
      ...
      <Aux>
        ...
        <div className={classes.divOfTimes}>
          ...
          <div className={classes.divInput}>
            ...
            <TimeField
              ref={this.laReference}
              class={classes.timeFieldStyle}
              value={this.state.arrivalTime}
              onChange={(value) => this.arrivalTimeChanged(value)}
            />
          </div>
          ...
          <div className={classes.divInput}>
            ...
            <TimeField
              ...
              onChange={(value) => this.leavingTimeChanged(value)}
            />
          </div>
        </div>
        ...
        <div className={classes.divOfTimes}>
          ...
          <div className={classes.divInput}>
            ...
            <div>
              <TimeField
                class={classes.timeFieldStyle}
                value={this.state.startOfLunch}
                onChange={(value) => this.startLunchChanged(value)}
              />
            </div>
          </div>
          ...
          <div className={classes.divInput}>
            ...
            <div>
              <TimeField
                class={classes.timeFieldStyle}
                value={this.state.endOfLunch}
                onChange={(value) => this.endLunchChanged(value)}
              />
            </div>
          </div>
        </div>
